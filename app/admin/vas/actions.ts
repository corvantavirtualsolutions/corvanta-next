...
export async function updateVA(
  id: string,
  formData: FormData
  ...
  let profileImageUrl: string | null = existingImageUrl;
  ...
  const { error } = await adminClient
    ...
    .update({
      ...
      iq: iqRaw ? Number(iqRaw) : null,
      english_score:
        (formData.get("english_score") as string | null)?.trim() || null,
      portfolio_link:
        (formData.get("portfolio_link") as string | null)?.trim() || null,
      facebook_link:
        (formData.get("facebook_link") as string | null)?.trim() || null,
      linkedin_link:
        (formData.get("linkedin_link") as string | null)?.trim() || null,
      profile_image_url: profileImageUrl,
    })
    .eq("id", id);
